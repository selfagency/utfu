...
var windows1252 = createCommonjsModule(function (module, exports) {
...
(function (root) {
  ...
  var decode = function (input, options) {
    ...
    while (++index < length) {
      byteValue = input.charCodeAt(index); // “If `byte` is in the range `0x00` to `0x7F`, return a code point whose
      // value is `byte`.”
  ...
  var encode = function (input, options) {
    ...
    while (++index < length) {
      codePoint = input.charCodeAt(index); // “If `code point` is in the range U+0000 to U+007F, return a byte whose
      // value is `code point`.”
      ...
      if (hasOwnProperty.call(INDEX_BY_CODE_POINT, codePoint)) {
        pointer = INDEX_BY_CODE_POINT[codePoint]; // “Return a byte whose value is `pointer + 0x80`.”
...
const hex = str => {
  ...
  str = windows1252.decode(windows1252.encode(str));
